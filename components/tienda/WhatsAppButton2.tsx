...
import { formatPrice, formatSku, abrirWhatsApp } from '@/lib/utils';
  ...
  return (
    <button
      ...
      className={`w-full py-3.5 rounded-2xl text-white font-bold flex items-center justify-center gap-2 transition-all ${
        !talla ? 'bg-gray-300' : 'shadow-lg active:scale-95'
      }`}
      style={talla ? { background: 'linear-gradient(135deg, #25D366, #128C7E)' } : {}}
    >
      ...
      {talla ? '¡Lo quiero!' : 'Selecciona talla'}
    </button>
  );
}